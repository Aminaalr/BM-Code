...
def loadDataset():
    ...
    x_train, x_test, y_train, y_test = train_test_split(imgArr, labelArr, test_size=test_ratio,random_state=2, stratify=labelArr)
    print('Read complete')
    print(len(x_train))
    print(len(x_test))
...
def Gnet_capsNet_model(n_routings=3):
    ...
    GNnet =tf.keras.applications.InceptionV3(include_top=False,weights="imagenet",input_shape=(image_size,image_size,3))
...
history = GNnetCapsNet_Model.fit(x_train,y_train,validation_data=(x_test, y_test),epochs=50,verbose=1,batch_size=100,callbacks=[log, checkpoint, reduce_lr])
